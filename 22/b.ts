...
for (let i = 0; i < bricks.length; i++) {
  let trimmedBricks = [...bricks];
  trimmedBricks.splice(i, 1);
  let unstable = true;
  while (unstable) {
    [unstable, trimmedBricks] = move(trimmedBricks);
  }
  let mm = 0;

  const referenceBricks = [...bricks];
  referenceBricks.splice(i, 1);
  for (let j = 0; j < trimmedBricks.length; j++) {
    if (trimmedBricks[j].toString() !== referenceBricks[j].toString()) {
      mm += 1;
    }
  }
  ans += mm;
